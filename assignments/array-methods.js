...
let fullNames = [];
runners.forEach(cv => {
  fullNames.push(cv.first_name + " " + cv.last_name);
});
...
runners.map(cv => {
  firstNamesAllCaps.push(cv.first_name.toUpperCase());
});
...
// Problem 1
// Create an array of the runners' emails, so the promotions department may send them their completion certificate.

let promotionEmails = [];
runners.forEach(cv => {
  promotionEmails.push(cv.email);
});
console.log(promotionEmails);

// Problem 2
// The promotions department wants to give something special to those who donated more than $200. Create an array with the full names of those who donated more than $200

let bigBoiReward = [];
runners.forEach(cv => {
  if (cv.donation > 200) {
    bigBoiReward.push(cv.first_name + " " + cv.last_name);
  }
});
console.log(bigBoiReward);

// Problem 3
// Create a new array of the companies with representatives present and sort it alphabetically so that an attendance / sponsorship poster can be made.

let companyA2Z = [];
runners.forEach(cv => {
  companyA2Z.push(cv.company_name);
});

console.log(companyA2Z.sort());
